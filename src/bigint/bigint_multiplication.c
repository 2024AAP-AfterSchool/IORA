...
{   
    ...
    for (int i = 0; i < B->wordlen; i++)
    {
        ...
        for (int j = 0; j < (tmp_A->wordlen / 2); j++)
        {
            ...
            bi_mul_AB(&T0, &(tmp_A->start[2 * j]), &(B->start[i]));
        
            tmp_T0->start[2 * j] = T0->start[0];
            tmp_T0->start[(2 * j) + 1] = T0->start[1];
            bi_mul_AB(&T1, &(tmp_A->start[(2 * j) + 1]), &(B->start[i]));
...
res bi_mul_karatsuba(OUT bigint** dst, IN bigint* A, IN bigint* B)
{
    ...
    bi_delete(&tmp_result); bi_delete(&tmp_A); bi_delete(&tmp_B);
    END_TIMER(result, print_success_mul_karatsuba());
    return result;
}
...
{   
    ...
    END_TIMER(result, print_success_square_AA());
    return result;
}
...
{   
    ...
    END_TIMER(result, print_success_square_C());
    return result;
}
...
{   
    ...
    if (tmp_A == NULL || tmp_A->wordlen == 0)
    {
        ...
        END_TIMER(result, print_success_square_KARATSUBA());
        return result;
    }
    if (A->wordlen == 1)
    {
...
{   
    ...
    END_TIMER(result, print_success_square());
    return result;
}